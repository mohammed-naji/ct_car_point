    ...
    <div class="card">
        <div class="card-body">
            <form action="{{ route('dashboard.roles.update', $role->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="row">
                    <div class="col-md-6">
                        <x-input name="name_en" label="English Name" placeholder="Enter Role English Name"
                            value="{{ $role->name_en }}" />
                    </div>
                    <div class="col-md-6">
                        <x-input name="name_ar" label="Arabic Name" placeholder="Enter Role Arabic Name"
                            value="{{ $role->name_ar }}" />
                    </div>
                </div>
                ...
                <ul class="list-unstyled" style="column-count: 2">
                    @foreach ($permissions as $per)
                        <li><label><input type="checkbox" name="permissions[]" @checked($role->permissions->find($per->id))
                                    value="{{ $per->id }}">
